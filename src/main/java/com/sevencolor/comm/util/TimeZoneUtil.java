...
public class TimeZoneUtil {
	 ...
	 */
	public static String getTimeDifferenceInHoursString(TimeZone firstTimeZone, TimeZone secondTimeZone) {
	 ...
	 */
	public static double getTimeDifferenceInHours(TimeZone firstTimeZone, TimeZone secondTimeZone) {
	 ...
	 * @return 日期
	 */
	private static String appendTimeZoneSuffix(long offset, boolean isNeedGMTPrefix) {
		// 将偏移量转化为小时（小数去除不要）
	 ...
	 */
	public static Calendar transformTimeZone(String sourceDateString, TimeZone sourceTimeZone, TimeZone targetTimeZone)
			throws ParseException {
		Date sourceDate = DateUtil.transformStrToDate(sourceDateString);
		Calendar calendar = transformTimeZone(sourceDate, sourceTimeZone, targetTimeZone);
		return calendar;
	}
	 ...
	 */
	public static Calendar transformTimeZone(Date sourceDate, TimeZone sourceTimeZone, TimeZone targetTimeZone) {
		// 校验入参是否合法
	 ...
	 * @return Calendar 转化时区后的时间
	 */
	public static Calendar transformTimeZone(Calendar sourceDate, TimeZone sourceTimeZone, TimeZone targetTimeZone) {

		return transformTimeZone(sourceDate.getTime(), sourceTimeZone, targetTimeZone);
	}
	 ...
	 * @throws ParseException
	 */
	public static Calendar transformTimeZoneToUTC(String sourceTime, TimeZone sourceTimeZone) throws ParseException {
		Date sourceDate = DateUtil.transformStrToDate(sourceTime);
		return transformTimeZone(sourceDate, sourceTimeZone, TimeZoneUtil.getUTC());
	}
	 ...
	 * @return Calendar 转化成UTC时区后的Calendar格式的时间
	 */
	public static Calendar transformTimeZoneToUTC(Date sourceDate, TimeZone sourceTimeZone) {
		return transformTimeZone(sourceDate, sourceTimeZone, TimeZoneUtil.getUTC());
	}
	 ...
	 * @return Calendar 转化成UTC时区后的Calendar格式的时间
	 */
	public static Calendar transformTimeZoneToUTC(Calendar sourceDate, TimeZone sourceTimeZone) {
		return transformTimeZone(sourceDate.getTime(), sourceTimeZone, TimeZoneUtil.getUTC());
	}
	...
	public static Calendar transformChinaTimeZoneToPST(Calendar sourceDate) {
		return transformTimeZone(sourceDate.getTime(), TimeZoneUtil.getChinaTimeZone(), TimeZoneUtil.getPST());
	}
	 ...
	 * @throws ParseException
	 */
	public static Calendar transformChinaTimeZoneToPST(String sourceDate) throws ParseException {
		return transformTimeZone(DateUtil.transformStrToDate(sourceDate), TimeZoneUtil.getChinaTimeZone(),
				TimeZoneUtil.getPST());
	}
	 ...
	 * @throws ParseException
	 */
	public static Calendar transformPSTToChinaTimeZone(String sourceDate) throws ParseException {
		return transformTimeZone(DateUtil.transformStrToDate(sourceDate), TimeZoneUtil.getPST(),
				TimeZoneUtil.getChinaTimeZone());
	}
	 ...
	 * B>11月的第一个星期日</B>。 美国夏时制实行与否，完全由各州各郡自己决定
	 * 
	 * @return
	 */
	 ...
	 * @throws ParseException
	 */
	public static boolean isDaylightTimeInUSA(String dateString) throws ParseException {
		Date date = DateUtil.transformStrToDate(dateString);
	...
	public static boolean isDaylightTimeInUSA(Date dateNeedCheck) {
		...
		// 3月的第二个星期日
		Calendar daylightSavingTimeBeginDay = DateUtil.getSecondSundayOfMonth(year, 3);
		// 11月的第一个星期日
		Calendar daylightSavingTimeEndDay = DateUtil.getFirstSundayOfMonth(year, 11);
		...
		if (startDayInSecond <= dateNeedCheckInSecond && dateNeedCheckInSecond <= endDayInSecond) {
			return true;
		}
		return false;
	}
	 ...
	 * @throws ParseException
	 */
	public static boolean isInDaylightTime(TimeZone timeZone, String dateString) throws ParseException {
		Date date = DateUtil.transformStrToDate(dateString);
	 ...
	 * @throws ParseException
	 */
	public static boolean isInDaylightTime(TimeZone timeZone, Date date) throws ParseException {
		return timeZone.inDaylightTime(date);
	}
	 ...
	 * @throws ParseException
	 */
	public static boolean isInDaylightTime(String dateString) throws ParseException {
		Date date = DateUtil.transformStrToDate(dateString);
